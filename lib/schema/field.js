...
function getComp(comp, components, ctlProps, props) {
    if (Array.isArray(comp)) {
        ...
        if (ctlProps.error) {
            ...
            if (!props.props.className.match('error'))
 ...
 */
function Field(_a) {
    var label = _a.label, components = _a.components, comp = _a.comp, layout = _a.layout, props = __rest(_a, ["label", "components", "comp", "layout"]);
    var Form = components['Form'] || {
        "Item": constant_1.DefaultItem
    };
    var ctlProps = getCtlProps(props.formal, props);
    return (react_1.default.createElement(Form.Item, __assign({ label: label }, layout), getComp(comp, components, ctlProps, props)));
}
exports.Field = Field;
/**
 * 对html tag特殊处理
